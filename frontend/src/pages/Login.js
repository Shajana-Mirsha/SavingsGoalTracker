...
// Natural water ripple canvas hook
function useRippleCanvas() {
  ...
  const addRipple = useCallback((x, y) => {
    // Spawn 3 staggered rings per event for a natural water look
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ripples.current.forEach(r => {
        // Decelerate as ring expands (natural water)
        const progress = r.r / r.maxR;
        r.r += r.speed * (1 - progress * 0.6);
        r.alpha *= 0.96;

        const rx = r.r;
        const ry = r.r * 0.38; // flat ellipse = water surface perspective
        ...
        // Subtle inner shimmer fill on first ring
        if (r.lineW > 1.5) {
...
export default function Login() {
  ...
  const handleMouseMove = useCallback((e) => {
    const now = Date.now();
    if (now - lastMove.current < 60) return; // Slightly throttled — still dense
      ...
      <div className="login-card" style={{
        ...
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <p style={{ margin: 0, color: "#94a3b8", fontSize: "14px", fontWeight: "500" }}>
            Sign in to your Vault Goal account
          </p>
        </div>

        {/* Success */}
        {success && (
          <div style={{ background: "rgba(22,101,52,0.3)", border: "1px solid rgba(34,197,94,0.4)", borderRadius: "12px", padding: "12px 16px", color: "#86efac", fontSize: "13px", fontWeight: "700", marginBottom: "20px", textAlign: "center" }}>
            ✓ {success}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} style={{ display: "grid", gap: "14px" }}>
          <div style={{ position: "relative" }}>
            ...
            <input
              className="pro-input"
              style={{ paddingLeft: "44px", background: "rgba(15,23,42,0.5)", border: "1px solid rgba(255,255,255,0.12)", color: "#f1f5f9", borderRadius: "14px" }}
        ...
        <p style={{ marginTop: "22px", textAlign: "center", color: "#64748b", fontSize: "14px", fontWeight: "600" }}>
          New here?{" "}
          <Link to="/register" style={{ color: "#818cf8", fontWeight: "800", textDecoration: "none" }}>Create Account</Link>
        </p>
      </div>
    </div>
  );
